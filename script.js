...
function downloadPDF() {
    ...
    if (logoImg && logoImg.complete && logoImg.naturalHeight !== 0) {
        doc.addImage(logoImg, 'PNG', 15, 10, 45, 28); // Wider and shorter
    } else if (typeof SCHOOL_LOGO_BASE64 !== 'undefined' && SCHOOL_LOGO_BASE64) {
        doc.addImage(SCHOOL_LOGO_BASE64, 'PNG', 15, 10, 45, 28); // Wider and shorter
    } else {
    ...
    yPos = doc.lastAutoTable.finalY + 6; // Reduced from 8
    ...
    doc.text("Class Teacher", 40, pageHeight - 25, { align: 'center' });
    doc.text("Campus Stamp", 85, pageHeight - 25, { align: 'center' });
    doc.text("Head of School", 130, pageHeight - 25, { align: 'center' });
    doc.text("Date", 175, pageHeight - 25, { align: 'center' });

    addLine(25, pageHeight - 28, 55, pageHeight - 28);
    addLine(70, pageHeight - 28, 100, pageHeight - 28);
    addLine(115, pageHeight - 28, 145, pageHeight - 28);
    addLine(160, pageHeight - 28, 190, pageHeight - 28);
    ...
    try {
        ...
        const doc = new Document({
            sections: [{
                ...
                children: [
                    // Header Table (Logo | Centered Text)
                    new Table({
                        rows: [
                            new TableRow({
                                children: [
                                    new TableCell({
                                        children: [
                                            new Paragraph({
                                                children: [logoImage ? logoImage : new TextRun("")],
                                    ...
                                    new TableCell({
                                        children: [
                                            new Paragraph({
                                                children: [
                                                    new TextRun({ text: "The Smart School", bold: true, size: 36, color: "E31E24", font: "Times New Roman", italics: true })
                                                ],
